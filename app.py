...
TMDB_API_KEY = "09064ce99d3e0ce44969ac60940cfe5a"  # Ganti dengan API key Anda
TMDB_BASE_URL = "https://api.themoviedb.org/3"

def get_movie_poster(movie_title, year=None):
    """
    Mendapatkan URL poster film dari TMDB API
    """
    try:
        # Search movie
        ...
        # Jika poster tidak ditemukan, gunakan placeholder
...
def load_movie_data():
    try:
        # Baca CSV dengan quoting untuk menangani tanda koma dalam teks
        df = pd.read_csv("movies.csv", quoting=1)  # quoting=1 means QUOTE_ALL
...
def get_mood_from_openrouter(text):
    """Helper function untuk mendapatkan mood menggunakan OpenRouter"""
    ...
    # Jika tidak ada keyword match, gunakan AI
    ...
    # Jika semua gagal, cek kata kunci terakhir
    if 'capek' in text_lower or 'rutinitas' in text_lower:
        return 'bosan'
        
    return 'bosan'  # Default untuk kasus ini

# Function untuk text classification (mood ke genre)
def classify_text_to_genre(text):
    # Define mood to genre mapping dengan bobot genre yang lebih spesifik
    mood_mapping = {
    ...
    # Dapatkan mood dan pastikan valid
...
# Function untuk rekomendasi film serupa
def get_similar_movies(movie_title, df):
    if movie_title not in df['title'].values:
        return pd.DataFrame()
        
    # Create TF-IDF vectors dari genre dan description
    tfidf = TfidfVectorizer(stop_words='english')
    
    # Berikan bobot lebih tinggi untuk genre dengan mengulanginya
    ...
    # Filter movies with similarity score > 0.3
    filtered_scores = [i for i in sim_scores[1:] if i[1] > 0.3]
    
    if not filtered_scores:
        return pd.DataFrame()
      # Get movie indices (skip first as it's the movie itself)
    movie_indices = [i[0] for i in filtered_scores[:3]]  # Limit to 3 movies
    
    # Get similar movies and add similarity score
    similar_movies = df.iloc[movie_indices].copy()
    similar_movies['similarity_score'] = [i[1] for i in filtered_scores[:3]]  # Match the limit
    
    # Sort by both similarity and rating dengan bobot yang seimbang
...
def display_movie_recommendations(recommendations):
    if not recommendations.empty:
        ...
        for idx, (_, movie) in enumerate(recommendations.iterrows()):
            with cols[idx % 3]:
                # Poster dengan width yang disesuaikan
                poster_url = get_movie_poster(movie['title'], str(movie['year']))
                st.image(poster_url, use_container_width=True)
                
                # Info film dengan style tengah
...
def main():    # Center the header image
    ...
    # Create tabs with centered content
